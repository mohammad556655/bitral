...
class StochasticRSI(IndicatorUtils):
    def analyze(self, historical_data, period_count=8,
        ...
        rsi = abstract.RSI(historical_data, period_count)

        stochrsi  = (rsi - rsi.rolling(5).min()) / (rsi.rolling(5).max() - rsi.rolling(5).min())
        ...
        stoch_rsi = pandas.concat([historical_data, kd_values], axis=1)
