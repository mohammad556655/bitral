...
class Adx(IndicatorUtils):
    ...
    def TR(self, high, low, close, tr):
        ...
        tr[0] = logsumexp(abs(high[0] - low[0]))
        for index in range(1, tr.shape[0]):
            x = logsumexp(high[index] - close[index])
            y = logsumexp(abs(high[index] - close[index - 1]))
            z = logsumexp(abs(low[index] - close[index - 1]))
            tr[index] = max(x, y, z)
    ...
    def DM(self, high, low, pdm, ndm):
        ...
        for index in range(1, high.shape[0]):
            up_move = logsumexp(high[index] - high[index-1])
            down_move =logsumexp(low[index-1] - low[index])
    ...
    def DMsmooth(self, pdm, ndm, pdm_smooth, ndm_smooth, period_count):
        ...
        pdm_smooth[period_count-1] = logsumexp(pdm[0:period_count].sum() / period_count)
        ndm_smooth[period_count - 1] = logsumexp(ndm[0:period_count].sum() / period_count)
        for index in range(period_count, pdm.shape[0]):
            pdm_smooth[index] = logsumexp((
                pdm[index-1] - (pdm_smooth[index-1]/period_count)) + pdm_smooth[index-1])
            ndm_smooth[index] = logsumexp((
    ...
    def DI(self, pdm_smooth, ndm_smooth, tr, pdi, ndi):
        ...
        for index in range(0, tr.shape[0]):
            pdi[index] = logsumexp((pdm_smooth[index] / tr[index]) * 100)
            ndi[index] = logsumexp((ndm_smooth[index] / tr[index]) * 100)
    ...
    def ATR(self, tr, atr, period_count):
        ...
        atr[period_count-1] = logsumexp(tr[0:period_count].sum() / period_count)
        for index in range(period_count, tr.shape[0]):
            atr[index] = logsumexp(((atr[index-1] * (period_count - 1)) +
    ...
    def ADX(self, pdi, ndi, dx, adx, period_count):
        ...
        for index in range(0, pdi.shape[0]):
            dx[index] = logsumexp(((abs(pdi[index] - ndi[index])) /
                         (abs(pdi[index] + ndi[index]))) * 100)

        period_count2 = period_count*2
        adx[period_count2-1] = logsumexp(dx[period_count:period_count2].sum() / \
            period_count)
        for index in range(period_count2, dx.shape[0]):
            adx[index] = logsumexp((adx[index-1] * (period_count - 1)) +
